...
class GameState:
        ...
        for i, unit_types in enumerate(units):
            for uinfo in unit_types:
                ...
                if unit_type == REMOVE:
                    ...
                    unit = GameUnit(unit_type, self.config,
                                    player_number, hp, x, y)
    ...
    def _invalid_player_index(self, index):
        self.warn(
            "Invalid player index {} passed, player index should always be 0 (yourself) or 1 (your opponent)".format(index))
    ...
    def get_resource(self, resource_type, player_index=0):
        ...
        if not resource_type == self.MP and not resource_type == self.SP:
            self.warn("Invalid resource_type '{}'. Please use MP (0) or SP (1)".format(
                resource_type))
            return
    ...
    def project_future_MP(self, turns_in_future=1, player_index=0, current_MP=None):
        ...
        if turns_in_future < 1 or turns_in_future > 99:
            self.warn("Invalid turns in future used ({}). Turns in future should be between 1 and 99".format(
                turns_in_future))
        if not player_index == 1 and not player_index == 0:
            self._invalid_player_index(player_index)
        if type(current_MP) == int and current_MP < 0:
            self.warn(
                "Invalid current MP ({}). Current MP cannot be negative.".format(current_MP))

        MP = self.get_resource(
            self.MP, player_index) if not current_MP else current_MP
    ...
    def can_spawn(self, unit_type, location, num=1):
        ...
        if not self.game_map.in_arena_bounds(location):
            if self.enable_warnings:
                self.warn("Could not spawn {} at location {}. Location invalid.".format(
                    unit_type, location))
            return False
        ...
        on_edge = location in (self.game_map.get_edge_locations(
            self.game_map.BOTTOM_LEFT) + self.game_map.get_edge_locations(self.game_map.BOTTOM_RIGHT))
        ...
        if self.enable_warnings:
            ...
            if len(fail_reason) > 0:
                self.warn("Could not spawn {} at location {}.{}".format(
    ...
    def attempt_remove(self, locations):
        ...
        for location in locations:
            if location[1] < self.HALF_ARENA and self.contains_stationary_unit(location):
                ...
                self.warn(
                    "Could not remove a unit from {}. Location has no structures or is enemy territory.".format(location))
    ...
    def attempt_upgrade(self, locations):
        ...
        for location in locations:
            if location[1] < self.HALF_ARENA and self.contains_stationary_unit(location):
                ...
                self.warn(
                    "Could not upgrade a unit from {}. Location has no structures or is enemy territory.".format(location))
        return spawned_units
    ...
    def find_path_to_edge(self, start_location, target_edge=None):
        ...
        if self.contains_stationary_unit(start_location):
            self.warn("Attempted to perform pathing from blocked starting location {}".format(
                start_location))
            return
    ...
    def get_target(self, attacking_unit):
        ...
        if not isinstance(attacking_unit, GameUnit):
            self.warn("Passed a {} to get_target as attacking_unit. Expected a GameUnit.".format(
                type(attacking_unit)))
            return
    ...
    def get_positions(self):
        dims = self.game_map.ARENA_SIZE
        board = np.array(-np.ones((dims, dims, 2)))
        state_data = json.loads(self.serialized_string)

        for player_units in ('p1Units', 'p2Units'):
            for unit_type in range(len(STRUCTURE_TYPES)):
                for unit in state_data[player_units][unit_type]:
                    board[unit[0], unit[1]] = [unit_type, unit[2]]

        # sides = np.array((board[:self.game_map.HALF_ARENA,:,:], board[self.game_map.HALF_ARENA:,:,:]))
        # debug_write(sides.shape)
        return board.copy()
